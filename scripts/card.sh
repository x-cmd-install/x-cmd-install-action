#!/usr/bin/env bash
# scripts/card.sh — generate data/<YYMMDD>.yml for an upstream repo.
#
# Standalone jq-based port of `x repo card`. Reads owner-repo from
# $1 (or GITHUB_REPOSITORY if unset), days from $2 (default 90).
# Writes YAML to stdout.
#
# Cost: 1 GraphQL request (16 search counts merged) + 1 REST
# /stats/commit_activity. Both well inside the 1000 req/h that
# GITHUB_TOKEN gets per repo.
#
# Requires: bash, curl, jq >= 1.6, and a token in GH_TOKEN/GITHUB_TOKEN.

set -eu

here="$(cd "$(dirname "$0")" && pwd)"

owner_repo="${1:-${GITHUB_REPOSITORY:-}}"
days="${2:-90}"
...
# jq's -L / include resolves relative to CWD, not the -L path, on
# jq 1.8 — so inline the module text into every filter instead.
